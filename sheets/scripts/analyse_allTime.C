...
//    }
    ...
    int TonjesPrint(){
       ...
       if (activity() == 1 && majorityUS_LPCnew() && year()>=LeastYear() ){
       	std::cout << "analyse_allTime.C plot published analysis: " << code << ", total auth: " << anAuth << ", anAuthUSA: " << anAuthUSA << ", anAuthLPCnew: " << anAuthLPCnew << ", anAuthUSA/anAuth: " << USAfrac << ", anAuthLPCnew/anAuthUSA: " << LPC_frac << std::endl;
       	std::cout << "select analyse_allTime.C plot published analysis with >=0.5 LPCnew authors : " << code << ", booleans: majorityUS_nonLPCnew(): "<<majorityUS_nonLPCnew()<<", majorityUS(): " << majorityUS() << ", majorityUS_LPCnew(): " << majorityUS_LPCnew() <<", just for check: majorityLPCnew(): "<<majorityLPCnew()<< std::endl;
//       	std::cout << "analyse_allTime.C plot check pub, analysis: " << code << ", my version: MajAuthUS_notLPCnew(): "<<MajAuthUS_notLPCnew()<< ", MajAuthUS(): " << MajAuthUS() << ", MajAuthUSLPCnew(): " << MajAuthUSLPCnew() <<", MajAuthNotUS(): " << MajAuthNotUS()<<  std::endl;
       }
       if (activity() == 1 && significantUS_LPCnew() && year()>=LeastYear() ){       
       	std::cout << "select analyse_allTime.C plot published analysis with >=0.3 LPCnew authors : " << code << ", booleans: significantUS_nonLPCnew(): "<<significantUS_nonLPCnew()<<", significantUS(): " << significantUS() << ", significantUS_LPCnew(): " << significantUS_LPCnew() <<", just for check: significantLPCnew(): "<<significantLPCnew()<< std::endl;
       }
       ...
       if (activity() == 3 && majorityUS_LPCnew() && year()>=LeastYear()){
       	...
       	std::cout << "select analyse_allTime.C plot Active analysis with >=0.5 LPCnew authors : " << code << ", booleans: majorityUS_nonLPCnew(): "<<majorityUS_nonLPCnew()<<", majorityUS(): " << majorityUS() << ", majorityUS_LPCnew(): " << majorityUS_LPCnew() <<", just for check: majorityLPCnew(): "<<majorityLPCnew()<< std::endl;
//       	std::cout << "analyse_allTime.C plot check pub, analysis: " << code << ", my version: MajAuthUS_notLPCnew(): "<<MajAuthUS_notLPCnew()<< ", MajAuthUS(): " << MajAuthUS() << ", MajAuthUSLPCnew(): " << MajAuthUSLPCnew() <<", MajAuthNotUS(): " << MajAuthNotUS()<<  std::endl;
       }  
        if (activity() == 3 && significantUS_LPCnew() && year()>=LeastYear()){
       	std::cout << "select analyse_allTime.C plot Active analysis with >=0.3 LPCnew authors : " << code << ", booleans: significantUS_nonLPCnew(): "<<significantUS_nonLPCnew()<<", significantUS(): " << significantUS() << ", significantUS_LPCnew(): " << significantUS_LPCnew() <<", just for check: significantLPCnew(): "<<significantLPCnew()<< std::endl;
       }
       if (activity() == 2 && majorityUS_LPCnew() && year()>=LeastYear()){
       	std::cout << "analyse_allTime.C plot PAS-only analysis: " << code << ", total auth: " << anAuth << ", anAuthUSA: " << anAuthUSA << ", anAuthLPCnew: " << anAuthLPCnew << ", anAuthUSA/anAuth: " << USAfrac << ", anAuthLPCnew/anAuthUSA: " << LPC_frac << std::endl;
       	std::cout << "select analyse_allTime.C plot PAS-only analysis with >=0.5 LPCnew authors : " << code << ", booleans: majorityUS_nonLPCnew(): "<<majorityUS_nonLPCnew()<<", majorityUS(): " << majorityUS() << ", majorityUS_LPCnew(): " << majorityUS_LPCnew() <<", just for check: majorityLPCnew(): "<<majorityLPCnew()<< std::endl;
//       	std::cout << "analyse_allTime.C plot check pub, analysis: " << code << ", my version: MajAuthUS_notLPCnew(): "<<MajAuthUS_notLPCnew()<< ", MajAuthUS(): " << MajAuthUS() << ", MajAuthUSLPCnew(): " << MajAuthUSLPCnew() <<", MajAuthNotUS(): " << MajAuthNotUS()<<  std::endl;
       }  
       if (activity() == 2 && significantUS_LPCnew() && year()>=LeastYear()){
        	std::cout << "select analyse_allTime.C plot PAS-only analysis with >=0.3 LPCnew authors : " << code << ", booleans: significantUS_nonLPCnew(): "<<significantUS_nonLPCnew()<<", significantUS(): " << significantUS() << ", significantUS_LPCnew(): " << significantUS_LPCnew() <<", just for check: significantLPCnew(): "<<significantLPCnew()<< std::endl;
       }     
...
//	if (activity() == 1){
//		std::cout << "analyse_allTime.C plot check, analysis: " << code << ", my version IfCalculate: NoAuth: "<<NoAuth<<", CalcAuthUS_notLPCnew: " << CalcAuthUS_notLPCnew << ", CalcAuthLPCnew: " << CalcAuthLPCnew <<", CalcAuthNotUS: " << CalcAuthNotUS <<  std::endl;
 //   }
