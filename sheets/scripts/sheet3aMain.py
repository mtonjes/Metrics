...
#   
    ...
    mykey = ""
#    
    notes_ids = list(analysies_json[analysis_code]["notes"].keys())
    print("in CADI: %s" % analysis_code)
#    
    for note_id in notes_ids:
        if (note_id in annotes_json):
...
#                        print author["institute"]
    ...
    print("Has institutes values: %s" % list(institutes.values()))
    print("Has institutes keys: %s" % list(institutes.keys()))
...
#    print "Has US institutes: %s" % institutes_us
    ...
    output_line.append(", ".join(list(institutes.keys())))
