...
def load_usa_lpc_authors_csv():
    print("in load_usa_lpc_authors_csv")
    f = open('data/iCMS_New_All_15Aug2022_noonCDT_LPC_fakeSurvey.csv', 'r')
    print("opened data/iCMS_New_All_15Aug2022_noonCDT_LPC_fakeSurvey.csv")
    lines = f.read().split("\n")
    for line in lines[1:]:
        columns = line.split("|")
        if len(columns) > 6:
...
#
analysis_codes = list(analysies_json.keys())
analysis_codes.sort()
#
# TOTALS
...
for analysis_code in analysis_codes:
    print("going through cadi analyses in analysis_code")
    status = ""
    samples = ""
    arc_chair_from_USA = "-"
    arc_chair_from_LPC = "-"
    ...
    if len(list(chairperson.keys())) > 0:
        # arc_chair_from_USA
        ...
        # arc_chair_from_LPC_N###
        if chairperson['fullname'] in usa_lpc_authors and usa_lpc_authors[chairperson['fullname']]:
    ...
    if len(list(cadi_contact.keys())) > 0:
        # cadi_contact_from_USA
        ...
        # cadi_contact_from_LPC_N###
        if cadi_contact['fullname'] in usa_lpc_authors and usa_lpc_authors[cadi_contact['fullname']]:
    ...
    for member_key in arc_members :
        ...
        if member_key in usa_lpc_authors and usa_lpc_authors[member_key]:
    ...
    #
    for note_key in list(analysis["notes"].keys()):
        if note_key in annotes_json:
            ...
            if fname_name in usa_lpc_authors and usa_lpc_authors[fname_name]:
            ...
            #
                #    institutes_num
                if author["institute"] not in institutes:
                ...
                if author["country"] == "USA":
                    ...
                    if author["institute"] not in institutes_USA:
                ...
                if isInLPC(author["institute"]):
                    ...
                    if author["institute"] not in institutes_LPC:
                        institutes_LPC[author["institute"]] = author["country"]
                #
                if author_key in usa_lpc_authors and usa_lpc_authors[author_key]:####
                    #    institutes_num_from_LPC_N###
                    if author["institute"] not in institutes_LPC_N:###
